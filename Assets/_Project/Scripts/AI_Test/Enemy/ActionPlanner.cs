...
namespace AITest.Enemy
{
    ...
    public class ActionPlanner : MonoBehaviour
    {
        ...
        public void Execute(RLAction action, string targetSectorId = null)
        {
            ...
            {
                if (debugMode)
                    Debug.Log($"<color=orange>[ActionPlanner] ⚠️ INTERRUPTING {currentAction} for URGENT CHASE!</color>");
        ...
        private IEnumerator ExecuteGoTo(Vector2 target, string label)
        {
            ...
            if (isRealTimeChase)
            {
                ...
                {
                    Debug.Log($"<color=yellow>[ActionPlanner] ⚠️ Player lost from sight, going to last position...</color>");
            ...
            for (int i = 0; i < Mathf.Min(3, sweepRoute.Length); i++)
            {
                ...
                if (originalIndex >= 0 && sector.hidingStats != null && originalIndex < sector.hidingStats.Length)
                {
                    if (playerFound)
                    {
                        ...
                        Debug.Log($"<color=red>[HidingLearning] ⚠️ Player FOUND at {sectorId}-S{originalIndex}!</color>");
